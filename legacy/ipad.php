...
<body>
    ...
    <div id="sidebar-squad" class="sidebar glass">
        <div class="flex justify-between items-center mb-2">
            <h2 class="text-xl font-bold">Plantilla</h2>
            <button onclick="toggleSidebar()" class="p-1 bg-white/4 rounded-full">
                <i data-lucide="x"></i>
            </button>
        </div>
        <div class="overflow-y-auto flex-1 pr-1 space-y-1">
            <?php foreach ($socios as $s): ?>
                <div onclick="addPlayerManual(<?= $s['id'] ?>, '<?= addslashes($s['nombre']) ?>')"
                    id="player-<?= $s['id'] ?>"
                    class="bg-white/5 p-2 px-4 rounded-2xl flex items-center gap-2 active:scale-95 transition cursor-pointer hover:bg-white/10">
                    <div class="w-8 h-8 bg-[url('player.png')] bg-contain bg-no-repeat bg-center"></div>
                    <span class="text-xs font-semibold truncate flex-1"><?= htmlspecialchars($s['nombre']) ?></span>
                    <i data-lucide="plus" class="w-3 h-3 opacity-40"></i>
                </div>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    <div class="main-stage">
        ...
        <div id="flyout-tactics" class="tactics-flyout glass">
            <!-- LIBRE -->
            <button onclick="setFormation('Libre', [])" class="tactic-btn">
                <div class="mini-board" style="background: rgba(255,255,255,0.05);"></div>
                <span class="tactic-name">LIBRE</span>
            </button>
        ...
        <nav class="tab-bar glass">
            <div class="tab-item" onclick="toggleSidebar()">
                ...
                <span class="text-[10px] uppercase font-bold mt-1">Equipo</span>
            </div>
            ...
            <div class="tab-item" onclick="toggleTool('pencil')" id="tab-pencil">
                ...
                <span class="text-[10px] uppercase font-bold mt-1">Dibujo</span>
            </div>
            ...
            <div class="tab-item" onclick="undo()" id="btn-undo">
                <i data-lucide="undo-2"></i>
                <span class="text-[10px] uppercase font-bold mt-1">Deshacer</span>
            </div>
            <div class="tab-item" onclick="redo()" id="btn-redo">
                <i data-lucide="redo-2"></i>
                <span class="text-[10px] uppercase font-bold mt-1">Rehacer</span>
            </div>
            ...
            <div class="tab-item" onclick="clearBoard()">
                ...
                <span class="text-[10px] uppercase font-bold mt-1">Limpiar</span>
            </div>
    ...
    <script>
        ...
        function addPlayerManual(id, name) {
            ...
            if (playersCount === 0) disk.classList.add('gk-disk');
        ...
        function setFormation(name, coords) {
            ...
            // Reposicionar jugadores existentes si están en el campo
            const activePlayers = document.querySelectorAll('.player-disk');
            activePlayers.forEach((p, index) => {
                if (index < coords.length) {
                    p.dataset.baseX = coords[index][0];
                    p.dataset.baseY = coords[index][1];
                    p.dataset.line = categorizeLine(coords[index][1]);
                    if (index === 0) p.classList.add('gk-disk');
                    else p.classList.remove('gk-disk');
                }
            });
